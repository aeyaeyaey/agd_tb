...
class CustomWebsiteProfile(WebsiteProfile):
    ...
    def view_user_profile_edition(self, **kwargs):
        ...
        unis = request.env['agd.uni'].sudo().search([])
        ...
        faks = request.env['agd.fak'].sudo().search([('uni_id', '=', user.uni_id.id)]) if user.uni_id else []
        liseler = request.env['agd.lise'].sudo().search([('ilce_id', '=', user.ilce_id.id)]) if user.ilce_id else []
        bolumler = request.env['agd.bolum'].sudo().search([('fak_id', '=', user.fak_id.id)]) if user.fak_id else []
    ...
    def get_fak(self, uni_id):
        fakulteler = request.env['agd.fak'].sudo().search([('uni_id', '=', int(uni_id))])
    ...
    def get_bolum(self, fak_id):
        bolumler = request.env['agd.bolum'].sudo().search([('fak_id', '=', int(fak_id))])
